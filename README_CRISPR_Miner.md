...
- (Optional) Downstream structural and embedding passes:
  ...
  - AlphaFold3 on cluster representatives to obtain structures (automated if `--alphafold3-cmd` is provided).
  - FoldSeek on AlphaFold3 models to search structural space and find remote homologs (automated if FoldSeek is installed).
  - Phylogenetic tree on coarse cluster representatives (MAFFT + FastTree required; built automatically).
...
----------------------------
- MMseqs2 GPU build (CUDA-enabled) from https://github.com/soedinglab/MMseqs2.
- FoldSeek from https://github.com/steineggerlab/foldseek.
- ESM3 (embeddings) from https://github.com/evolutionaryscale/esm
- AlphaFold3 from https://github.com/google-deepmind/alphafold3
Install these separately and ensure they’re on PATH or referenced explicitly; they are not in this bundle.
...
-------------------
```bash
# Requires: python3, prodigal, CRONUS, CRISPRCasFinder, mmseqs (GPU build), mafft, fasttree/FastTree, writable tmp space
...
- ESM3 embeddings (example; adapt to your install):
  ```bash
  esm3_embed --fasta operon_proteins.faa --outdir esm3_embeddings --device cuda
  ```
- AlphaFold3 modeling: run externally on prioritized FASTAs or cluster reps; output PDBs to `af3_models/`.
